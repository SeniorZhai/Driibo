...
import com.zoe.driibo.util.CacheUtils;

public class RequestManager {
	public static RequestQueue mRequestQueue = newRequestQueue();

	// 取运行内存阈值的1/3作为图片缓存
	private static final int MEM_CACHE_SIZE = 1024 * 1024 * ((ActivityManager) MyApp
	...
	private static Cache openCache() {
		return new DiskBasedCache(CacheUtils.getExternalCacheDir(MyApp
					...
					if (!isImmediate && defaultImageDrawable != null) {
						TransitionDrawable transitionDrawable = new TransitionDrawable(
								new Drawable[] {
										...
										new BitmapDrawable(MyApp.getContext()
												.getResources(),
												response.getBitmap()) });
